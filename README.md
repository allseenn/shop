...
Данная инструкция посвящена второму заданию контрольной работы, объединенную мной в один проект ([**shop**](#startsh-исполнение-двух-задач-одним-скриптом)) с инклюзивным решением двух Задач:
...
Программный код каждой Задачи могут быть запущенны по отдельности, либо совместно. Архитектура обеих программ построена таким образом, что используются общие папки и файлы, в тоже время, ничто не мешает использовать их по отдельности.

## start.sh исполнение двух задач одним скриптом

Программы обеих Задач размещены на [гитхабе](https://github.com/allseenn/shop.git) и представляют одну директорию **shop**, в которой лежат:
...
Для запуска программ достаточно клонировать репозиторий в любую удобную директорию и запустить стартовый скрипт **start.sh**:
...
С небольшой паузой в 3 секунды, начинают выполняться все 8 заданий. Перед началом задания выводится его описательный текст. По окончании проделанный результат.
...
Но, можно запустить *java* программу в составе стартового скрипта *start.sh* выбрав цифру *2* главного меню.
Сначала запуститься сборка классов, через несколько секунд запуститься программа с *java*-меню:

<img src=pics/03.png>

*Java*-программа проверит наличие директорий оставшихся после задания с *Ubuntu*. Если файлов и папок нет, *java* создаст их автоматически, если они остались от предыдущего задания, *java* будет их использовать для хранения данных.
